...
from models import (anneal_Langevin_dynamics,
                    anneal_Langevin_dynamics_graph,
...
def get_model(config):
    if config.model.type == 'NCSNv1' and (config.data.dataset == 'CIFAR10' or config.data.dataset == 'CELEBA'):
        return NCSN(config).to(config.device)
    elif config.model.type == 'NCSNv1' and config.data.dataset == 'LSUN':
        return NCSNdeeper(config).to(config.device)
    elif config.model.type == 'NCSNv2' and (config.data.dataset == 'CIFAR10' or config.data.dataset == 'CELEBA'):
        return NCSNv2(config).to(config.device)
    elif config.model.type == 'NCSNv2' and config.data.dataset == "FFHQ":
        return NCSNv2Deepest(config).to(config.device)
    elif config.model.type == 'NCSNv2' and config.data.dataset == 'LSUN':
...
class NCSNRunner():
    ...
    def sample(self):
        ...
        score = torch.nn.DataParallel(score,device_ids = [3,4,6,7])
        # score = torch.nn.DataParallel(score) # uses all devices
        ...
        if not self.config.sampling.fid:
            if self.config.sampling.inpainting:
                ...
                if self.config.sampling.data_init:
                    ...
                    print("---------- Batch_size = {} --------- ".format(self.config.sampling.batch_size))
                    init_samples = torch.rand(self.config.sampling.batch_size, self.config.data.channels,
                                              self.config.data.image_size, self.config.data.image_size,
                                              device=self.config.device)
                    init_samples = data_transform(self.config, init_samples)
                print("----------------      Sampling    --------")
                # print("----------------      Gamma == {} -------- ".format(self.config.sampling.gamma))
                # print("----------------      L == {}     ---------".format(L))
                # print("----------------      T == {}     ---------".format(self.config.sampling.n_steps_each))
                all_samples = anneal_Langevin_dynamics(init_samples, score, sigmas,
                                                       self.config.sampling.n_steps_each,
                                                    #    self.args.T,
                                                       self.config.sampling.step_lr, verbose=True,
                                                       gamma = self.config.sampling.gamma,
                                                       final_only=self.config.sampling.final_only,
                                                       denoise=self.config.sampling.denoise)
                ...
                if not self.config.sampling.final_only:
                    ...
                    for i, sample in tqdm.tqdm(enumerate(all_samples), total=len(all_samples),
                        ...
                        save_image(image_grid, os.path.join(self.args.image_folder, 'image_grid_{}.png'.format(i)))
                        torch.save(sample, os.path.join(self.args.image_folder, 'samples_{}.pth'.format(i)))
                    imgs[0].save(os.path.join(self.args.image_folder, "movie.gif"), save_all=True, append_images=imgs[1:], duration=1+(5-self.config.fast_fid.n_steps_each)*100, loop=0)
                ...
                save_image(image_grid, os.path.join(self.args.image_folder,
                                                        'image_grid_{}.pdf'.format(self.config.sampling.ckpt_id)),format='pdf')
        ...
        for ckpt in tqdm.tqdm(range(self.config.fast_fid.begin_ckpt, self.config.fast_fid.end_ckpt + 1, 5000),
            ...
            for i in range(num_iters):
                ...
                all_samples = anneal_Langevin_dynamics(init_samples, score, sigmas,
                                                       ...
                                                       denoise=self.config.sampling.denoise,
                                                       gamma = self.config.fast_fid.gamma)
    ...
    def find_fid(self):
        ...
        # python main.py --config cifar10.yml -i cifar10/cal_fid --doc cifar10 --T 2 --find_fid
        from evaluation.fid_score import get_fid, get_fid_stats_path
        import pickle
        import shutil
        score = get_model(self.config)
        score = torch.nn.DataParallel(score,device_ids = [0,1,2,3,5,6,7])

        sigmas_th = get_sigmas(self.config)
        sigmas = sigmas_th.cpu().numpy()

        fids = {}
        gammas = [self.config.sampling.gamma]
        print(gammas)
        
        print("----------------- Sample and Calculating FID ----------------")
        for gamma in tqdm.tqdm(gammas,
            ...
            num_iters = self.config.fast_fid.num_samples // self.config.fast_fid.batch_size
            output_path = os.path.join(self.args.image_folder, 'T_{}_gamma_{}'.format(self.config.fast_fid.n_steps_each,gamma))
            os.makedirs(output_path, exist_ok=True)
            ...
            for i in range(num_iters):
                ...
                for id, sample in enumerate(final_samples):
                    ...
                    save_image(sample, os.path.join(output_path, 'sample_{}.png'.format(i*self.config.fast_fid.batch_size + id)))

            stat_path = get_fid_stats_path(self.args, self.config, download=True)
            fid = get_fid(stat_path, output_path)
            fids[gamma] = fid
            print("------- Number of sampled images == {} -----".format(i*self.config.fast_fid.batch_size + id))
            print("Gamma: {}, fid: {}".format(gamma, fid))
            shutil.rmtree(output_path)

        with open(os.path.join(self.args.image_folder, 'T_{}_fids.pickle'.format(self.config.fast_fid.n_steps_each)), 'wb') as handle:
            pickle.dump(fids, handle, protocol=pickle.HIGHEST_PROTOCOL)
    ...
    def find_graph_data(self):
        ...
        states = torch.load(os.path.join(self.args.log_path, 'best_checkpoint_with_denoising.pth'), map_location=self.config.device)
        ...
        anneal_Langevin_dynamics_graph(self.config,self.args,score, sigmas,
                                self.config.fast_fid.n_steps_each,
                                # self.args.T,
                                self.config.fast_fid.step_lr,
                                gamma = self.config.sampling.gamma,
