...
# python evaluation/nearest_neighbor.py --path exp/image_samples/cifar10/nearest_neighbors/T_5_g_0.4/samples_300000.pth -i exp/image_samples/cifar10/nearest_neighbors/T_5_g_0.4/nearest --dataset cifar10 --n_samples 8
# python evaluation/nearest_neighbor.py --path exp/image_samples/celeba/nearest_neighbors/T_5_g_0.4/samples_210000.pth -i exp/image_samples/celeba/nearest_neighbors/T_5_g_0.4/nearest --dataset celeba --n_samples 8
...
def get_nearest_neighbors(dataset, path, name, n_samples, k=10, cuda=True):
    ...
    data_features = []
    data = []
    #for x, _ in tqdm(dataset, desc="sweeping the whole dataset"): # cifar10
    for x in tqdm(dataset, desc="sweeping the whole dataset"): # celeba
        if cuda: x = x.cuda()
...
class CustomDataset(Dataset):
    def __init__(self, root, transform=None):
        ...
        self.file_list = os.listdir(root)[:30000]
...
if __name__ == '__main__':
    ...
    if args.dataset == 'church':
        ...
        # dataset = CelebA('exp/datasets/celeba', split='train', transform=transforms,download=True)
        dataset =  CustomDataset(root="/data4/home/manikantab/Diffusion_models/IADB/celebA_50k", transform=transforms)

    elif args.dataset == 'cifar10':

        dataset = CIFAR10('/data4/home/manikantab/Diffusion_models/CIFAR10/data', train=True, transform=ToTensor())
